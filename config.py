...
# Аксессуары: их материал/силуэт/крой/отделка/принт/цвет НЕ идут в частоты
# трендов и сигналы инфлюенсеров (иначе «Металлическая фурнитура» = 86% сумок,
# «Замша» = туфли). Категорийные тренды (Сумка, Обувь) при этом остаются.
...
SEASONAL_YOY_TOLERANCE = 0.9   # сезонная поправка: Q ≥ 90% значения того же
                               # месяца год назад → м/м-падение не считается спадом
