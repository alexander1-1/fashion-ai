"""
УСТАРЕЛО: индекс теперь строит reindex_fashion_clip.py (Fashion-CLIP).
Этот скрипт оставлен для справки — он собирал старый clip_index.npy
на ванильном clip-ViT-B-32. Не запускать: перезапишет легаси-файлы,
которые служат фолбэком на HF Space.
